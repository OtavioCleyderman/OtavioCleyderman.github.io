...
//----------------------------------------------------------------------------------------//
// Inserção do Algoritmo de busca, exigido como parte do trabalho 02 de Estrutura de Dados// 
// A busca binária só funciona com uma lista preordenada        //
...
function buscaBinaria(vet, busca, fnComp, posIni = 0, posFim = vet.length - 1) {
  ...
  function comparaValorLinha(valor_buscado, objeto) {
    if (valor_buscado == objeto[0]) {
      ...
      return -1; // O resultado não foi encontrado //
    }
  }
  ...
  function escoreZ() {
    ...
    if (intervalo_selecionado === "maior" || intervalo_selecionado === "menor") {
      ...
      if (z >= 4) {
        ...
        const linha_com_valor = buscaBinaria(
          tabela_z_escore,
          z.slice(0, 3),
          comparaValorLinha
        );
        const indice = parseInt(z.slice(3, 4)) + 1;
        z_escore = linha_com_valor[indice]; // retorna o valor no índice correto //
      }
    }
  ...
  function entreValores() {
    let media = document.getElementById("media").value;
    let dados = document.getElementById("valor_intervalo").value;
    ...
    if (dados_split[0] == media || dados_split[1] == media) {
      ...
      z = ((valor_intervalo - media) / desvio_padrao).toFixed(2);
      z = z.replace("-", "");
      let linha_com_valor = buscaBinaria(
        tabela_z_escore,
        z.slice(0, 3),
        comparaValorLinha
      );
